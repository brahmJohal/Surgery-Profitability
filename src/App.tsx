...
const isFullSurgery = (surgery: SurgerySummary): surgery is Surgery =>
  'pharma_cost' in surgery

const fixedCost = (surgery: Surgery) =>
  surgery.pharma_cost +
  surgery.anaesthesia_cost +
  surgery.lab_cost +
  surgery.base_rental +
  surgery.other_fixed_cost

const surgeryCosts = (surgery: Surgery): CostFields => ({
  standard_billing: surgery.standard_billing,
  pharma_cost: surgery.pharma_cost,
  anaesthesia_cost: surgery.anaesthesia_cost,
  lab_cost: surgery.lab_cost,
  base_rental: surgery.base_rental,
  other_fixed_cost: surgery.other_fixed_cost,
})
...
function CostInputs({
  ...
  costs: CostFields
  setCosts: (value: CostFields) => void
}) {
  const set = (key: keyof CostFields) => (value: number) =>
    setCosts({ ...costs, [key]: value })

  return (
    <div className="three">
      <AmountInput label="Standard billing" value={costs.standard_billing} onChange={set('standard_billing')} />
      <AmountInput label="Pharma" value={costs.pharma_cost} onChange={set('pharma_cost')} />
      <AmountInput label="Anaesthesia" value={costs.anaesthesia_cost} onChange={set('anaesthesia_cost')} />
      <AmountInput label="Lab" value={costs.lab_cost} onChange={set('lab_cost')} />
      <AmountInput label="Base rental" value={costs.base_rental} onChange={set('base_rental')} />
      <AmountInput label="Other fixed costs" value={costs.other_fixed_cost} onChange={set('other_fixed_cost')} />
    </div>
  )
}
...
export default function App() {
  const [role, setRole] = useState<UserRole | null>(() =>
    !supabase ? getLocalSession()?.role || null : null,
  )
  ...
  useEffect(() => {
    ...
    const applySession = async (session: { user: { id: string } } | null) => {
      setRole(session ? await roleFromSession(session.user.id) : null)
    }
  ...
  useEffect(() => {
    ...
    getSurgeries(role)
      .then(items => {
        ...
        setValues({
          ...empty,
          billing: first?.standard_billing || 0,
          referralShare: role === 'online_sales' ? onlineSalesReferral : 0,
        })
      ...
      .catch(() => setStatus('Could not load surgeries.'))
      .finally(() => setLoading(false))
  }, [role])

  const surgery = surgeries.find(item => item.id === selectedId)

  const adminFixedCost =
    surgery && isFullSurgery(surgery) ? fixedCost(surgery) : 0
  ...
  useEffect(() => {
    ...
    calculateProfit(surgery.id, values)
      .then(next => {
        if (!stale) setResult(next)
      })
      .catch(() => {
        if (!stale) setStatus('Unable to calculate profit right now.')
      })
    ...
    values.billing,
    values.doctorShare,
    values.referralShare,
    values.extraRental,
  ])

  const update = (key: keyof Calculation) => (value: number) => {
  ...
  const choose = (id: string) => {
    ...
    setValues({
      ...empty,
      billing: item?.standard_billing || 0,
      referralShare: role === 'online_sales' ? onlineSalesReferral : 0,
    })
  ...
  const refresh = async () => {
    ...
    const items = await getSurgeries(role)
    setSurgeries(items)
  ...
  const save = async () => {
    ...
    try {
      await saveCalculation(surgery.id, values)
      setStatus('Calculation saved successfully.')
    } catch {
      setStatus('Could not save. Check your database setup.')
    }
  }
  ...
  const addSurgery = async () => {
    ...
    if (!name) {
      setStatus('Please enter the surgery name.')
      return
    }

    if (surgeries.some(item => item.name.toLowerCase() === name.toLowerCase())) {
      setStatus('This surgery already exists.')
      return
    }

    try {
      await saveSurgery({ name, ...newCosts, active: true })

      const items = await refresh()
      const added = items.find(item => item.name === name)

      if (added) choose(added.id)

      setNewName('')
      setNewCosts(blankCosts)
      setStatus('New surgery added successfully.')
    } catch {
      setStatus('Could not add surgery. Check your database setup.')
    }
  }
  ...
  const saveExisting = async () => {
    const existing = surgeries.find(item => item.id === editingId)

    if (!existing || !isFullSurgery(existing)) return
    ...
    try {
      ...
      await refresh()
      setEditingName(name)
      setStatus('Existing surgery updated successfully.')
    } catch {
      setStatus('Could not update this surgery.')
    }
  }
      ...
      <main>
        ...
        <div className="grid">
          <section className="card">
            ...
            {isOnlineSales && (
              <p className="form-help">
                Fixed referral share: {money(onlineSalesReferral)}
              </p>
            )}
          ...
          <aside className="card summary">
            ...
            {isAdmin && (
              <Metric label="Fixed procedure costs" value={'− ' + money(adminFixedCost)} />
            )}
            ...
            <p className="margin">
              Profit margin <b>{result.margin.toFixed(1)}%</b>
            </p>
          </aside>
        </div>
        ...
        <details className="manage">
          ...
          <div className="card manage-card">
            <p className="form-help">
              Enter the standard billing and all expected expenses for this new surgery.
            </p>
        ...
        {isAdmin && (
          <details className="manage" open>
            ...
            <div className="card manage-card">
              ...
              <button className="primary" onClick={saveExisting}>
